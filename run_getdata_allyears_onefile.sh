...
echo $run_dir
savedir='/cluster/work/users/sso102/S2S/ECMWF/TOT_PR_singlefiles' #needs to match the dir in getdata_TP_CY46R1_ECMWF.py
...
for d in ${DATE}; do 
    ...
    m=$(echo ${d} | cut -d'-' -f2)
    day=$(echo ${d} | cut -d'-' -f3)
    cp $run_dir/getdata_TP_CY46R1_ECMWF.py $run_dir/jobs.$$/getdata_TP_CY46R1_ECMWF_${d}.py 
    sed -i "s/2018-01-01/$d/g" $run_dir/jobs.$$/getdata_TP_CY46R1_ECMWF_${d}.py 
               
    if [ ! -f ${savedir}/TP_CY46R1_${d}_cf.grb ] ; then  #tp_cf_2019-07-15_hc_2000-07-15.nc
            echo "running python getdata_TP_CY46R1_ECMWF_${d}.py"
            python $run_dir/jobs.$$/getdata_TP_CY46R1_ECMWF_${d}.py
            wait
            echo "done..."
          else 
